...
const TodoList = () => {
  ...
  useEffect(() => {
    const filteringTodoListByDate = () => {
      ...
      switch (filterByDate) {
        case "期限前":
          setFilteredTodoList(
            filtered.filter((todo: Todo) => todo.deadlineStatus === "期限前")
          );
          break;

        case "今日":
          setFilteredTodoList(
            filtered.filter((todo: Todo) => todo.deadlineStatus === "今日")
          );
          break;

        case "期限切れ":
          setFilteredTodoList(
            filtered.filter((todo: Todo) => todo.deadlineStatus === "期限切れ")
          );
          break;
        default:
  ...
  const onClickMoveToCreate = async (
  ...
  return (
    <>
      <div className="flex flex-col space-y-10 justify-center items-center mb-5 mt-10">
        <div>
